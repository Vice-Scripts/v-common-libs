...
if isBridge then
    RegisterNetEvent('QBCore:Client:SetDuty', function(newDuty)
        local JobData = bridge.framework.getPlayerData().job

        JobData.onDuty = newDuty

        TriggerEvent("is_bridge:updateJob", JobData)
    end)
    
    RegisterNetEvent("QBCore:Client:OnPlayerLoaded", function()
        TriggerEvent("is_bridge:playerLoaded")
    end)

    RegisterNetEvent("QBCore:Client:OnPlayerUnload", function()
        TriggerEvent("is_bridge:playerUnloaded")
    end)
